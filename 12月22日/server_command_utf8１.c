...
//月本追加
static char gMoveDir[MAX_CLIENTS] = {0};
...
static Uint32 ServerGameLoop(Uint32 interval, void *param) 
{
    ...
    if (gMoveDir[i] == 0) continue;  // ★押してない

    int step = 10;

    if (gMoveDir[i] == DIR_UP)    gPlayerPosY[i] -= step;
    if(gPlayerPosY[i] <= 0)	gPlayerPosY[i] = 0;

    if (gMoveDir[i] == DIR_DOWN)  gPlayerPosY[i] += step;
    if(gPlayerPosY[i] >= 1000)	gPlayerPosY[i] = 1000;

    if (gMoveDir[i] == DIR_LEFT)  gPlayerPosX[i] -= step;
    if(gPlayerPosX[i] <= 0)	gPlayerPosX[i] = 0;

    if (gMoveDir[i] == DIR_RIGHT) gPlayerPosX[i] += step;
    if(gPlayerPosX[i] >= 1300)	gPlayerPosX[i] = 1300;

    //printf("ID %d x:%d y:%d\n", i, gPlayerPosX[i], gPlayerPosY[i]);
}
...
for (int i = 0; i < numClients; i++) {
    ...
    SetIntData2DataBlock(data, gPlayerPosX[i], &dataSize);
    SetIntData2DataBlock(data, gPlayerPosY[i], &dataSize);
}
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            switch (i) {
                case 0: // 左上
                    ...
                    break;
                case 1: // 右下
                    gPlayerPosX[i] = w - PADDING - SQ_SIZE;
                    gPlayerPosY[i] = h - PADDING - SQ_SIZE;
                    break;
                case 2: // 右上
                    gPlayerPosX[i] = w - PADDING - SQ_SIZE;
                    gPlayerPosY[i] = PADDING;
...
//
    ...
    switch(command){
		...
		int senderID = pos;
		char moveType;     // 押したか / 離したか
		char direction;    // どっちの方向か

		RecvCharData(senderID, &moveType);   // MOVE_PRESS or MOVE_RELEASE
		RecvCharData(senderID, &direction);  // DIR_UP, DIR_LEFT ...

		if (moveType == MOVE_PRESS)
			gMoveDir[senderID] = direction;     // ★長押し開始
		else
			gMoveDir[senderID] = 0;             // ★キーを離した
