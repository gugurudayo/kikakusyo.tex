...
int main(int argc, char *argv[]) {
    ...
    SDL_Texture* bg = IMG_LoadTexture(renderer, "iroiro1.png");
if (!bg) {
    ...
    while (!quit) {
        ...
        int shoot = 0;
        if (player_id == 0) {
        if (keys[SDL_SCANCODE_SPACE]) shoot = 1;   // 赤：スペースで発射（下向き）
        } else {
        if (keys[SDL_SCANCODE_RCTRL]) shoot = 1;   // 青：右Ctrlで発射（上向き）
        }
        move.shoot = shoot;
...
for (int owner = 0; owner < 2; owner++) {
    // 赤と青で弾の色を変えてもいい
    if (owner == 0) {
        SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255); // 赤側の弾: 黄色
    } else {
        SDL_SetRenderDrawColor(renderer, 0, 255, 255, 255); // 青側の弾: シアン
    }
    ...
    for (int k = 0; k < MAX_BULLETS; k++) {
        ...
        // （お好みで）ワールドの枠を描きたいなら、ここで計算してもいい

        // プレイヤー0と1を「ワールド座標 → 画面座標」に変換して描画
        SDL_Rect player0 = {
