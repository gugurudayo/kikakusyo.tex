...
\subsection{コンセプト・概要}
（記述例）無数の迫りくる敵（昆虫）に当たらずに，とにかく早くゴールをめざすハラハラドキドキのタイムバトル
\begin{itemize}
\item 強制横スクロール（毎秒10ピクセルでステージ（背景）をｘ軸マイナス方向に動かす）
\item プレイヤ（飛脚）は横移動とジャンプのみできる
\item 動きが異なる敵キャラは四方八方から出現してくる
\item 5分以内にゴールにたどり着けなければならない
\item 5ステージある（街中昼間，山間部，沿岸部，街中夜間，街中雨）
\item アイテムをとるとさまざまな効果を発揮できる
\end{itemize}

\subsection{世界観}
（記述例）江戸時代の日本．昆虫憐れみの令が発せられ，人々は昆虫（蚊など）を殺せなくなっただけでなく触れるだけでも罰せられるようになってしまった．飛脚である主人公の徳大夫（とくだゆう）は，今日も文を時間内に届なくてはならない．昆虫に触れないようにして．
